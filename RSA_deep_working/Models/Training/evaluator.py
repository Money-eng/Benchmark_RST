...
class Evaluator:
    def __init__(
        ...
        self.cpu_metrics_evaluation = False
    ...
    def evaluate(self, on_test: bool = False) -> dict:
            ...
            try:
                with tqdm(dataloader, desc="Evaluating batch per batch", leave=False, dynamic_ncols=True) as pbar:
                    ...
                    if self.cpu_metrics_evaluation:
                        ...
                        results_compute = compute(
                            *tasks, scheduler=self.client)
                ...
                if not self.do_not_reconstruct:
                    # if accuracy loss gives us more than 60% accuracy, we can process the whole series
                    f1_result = results_raw.get("f1_score", [])
                    f1_score = np.mean(f1_result) if f1_result else 0
                    if f1_score > 0.75:
                        with tqdm(dataloader, desc="Evaluating serie per serie", leave=False, dynamic_ncols=True) as pbar:
                            for ts_imgs, masks, _, mtgs in pbar:
                                ts_imgs = ts_imgs.to(self.device)
                                masks = masks.to(self.device)
                                if self.sw_inferer is None:
                                    preds_logits_sigmoidee = self.model(
                                        ts_imgs)
                                else:
                                    preds_logits_sigmoidee = self.sw_inferer(
                                        inputs=ts_imgs, network=self.model)

                                # mtg_gt, mtg_pred = process_date_map(mtgs, preds_logits_sigmoidee, jar_path=self.jar_path)
                                # mtg_gt, mtg_pred = process_date_map(mtgs, masks, jar_path=self.jar_path)
                                ...
                                for metric in self.mtg_metrics:
                                    name = metric.__class__.__name__
                                    pbar.set_postfix_str(f"Metric: {name}")
                                    results_raw[name].append(
                                        metric(mtg_pred, mtg_gt))
        ...
        return mean_results, results_raw
...
@delayed
def compute_mtg_from_images(mask, pred, jar_path):
    from utils.launch_RST import process_date_map
    mtg_gt, mtg_pred = process_date_map(mask, pred, jar_path=jar_path)
