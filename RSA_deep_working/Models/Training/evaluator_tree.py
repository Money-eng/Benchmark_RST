...
class Evaluator:
    ...
    def evaluate(self, on_test: bool = False) -> dict:
        ...
        with torch.no_grad():
            try:
                ...
                if not self.do_not_reconstruct:
                    ...
                    f1_result = results_raw.get("F1Score", [])
                    if len(f1_result) == 0:
                        f1_result = results_raw.get("f1_score", [])
                    f1_score = np.mean(f1_result) if f1_result else 0
                    if f1_score > 0.75:
                        with tqdm(dataloader, desc="Evaluating serie per serie", leave=False,
        ...
        mean_results = {name: np.mean(values) if values else 0.0 for name, values in results_raw.items()}
        return mean_results
