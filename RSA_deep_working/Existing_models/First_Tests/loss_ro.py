...
class HausdorffDTLoss(_Loss):
    ...
    @torch.no_grad()
    def distance_field(self, img: torch.Tensor,clip_value_in_voxels=4) -> torch.Tensor:
        ...
        for batch_idx in range(len(img)):
            ...
            if fg_mask.any() and not fg_mask.all():
                ...
                field[batch_idx] = torch.clamp(field[batch_idx], min=-clip_value_in_voxels, max=clip_value_in_voxels)   
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(input.shape[1]):
            ...
            pred_error = abs(ch_input - ch_target)# ** 2
